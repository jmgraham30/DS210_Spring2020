...
Let's determine how often (out of `r length(coffee)` total data values) the observed data point is less than the hypothesized mean value of 7:
```{r}
(obs_neg <- sum(coffee_minus_seven < 0))
```
We see that `r obs_neg` out of `r length(coffee)` of data points are less than 7. 
...
Let's look at the distribution of the number of negative values we get if we randomly assign a sign to `r length(coffee)` values:
```{r}
gf_bar(~sign_results) %>% gf_vline(xintercept = obs_neg,color="red")
```

The red vertical line indicates our observed value of `r obs_neg` negative values. We see that under the null hypothesis, we expect that most often we will get about the same number of positive values as negative. Let's compute the proportion of times that we get as many or more than `r obs_neg` negative values:
```{r}
